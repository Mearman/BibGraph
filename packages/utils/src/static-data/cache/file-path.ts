 ...
 *
 * Provides functions for generating cache file paths, static file paths, and managing cache entry metadata.
 */
...
const generateQueryFilePath = (pathSegments: readonly string[], queryString: string, staticDataRoot: string, url: string): string | null => {
	...
	if (!normalizedQuery || normalizedQuery === "?") {
		return generateBaseCollectionPath({ pathSegments: [...pathSegments], staticDataRoot })
	}
...
/**
 * Generate cache file path from OpenAlex URL This is the canonical mapping used by all systems
 */
export const getCacheFilePath = async ({
	...
	try {
		// Kept genuinely async (rather than dropping the async keyword) so this remains swappable for a real async implementation via the getCacheFilePathFn parameter default pattern used elsewhere in this package (e.g. collision-handling.ts).
...
/**
 * Generate static file path for service worker Maps OpenAlex URLs to static file paths for production serving
 */
export const getStaticFilePath = async (url: string): Promise<string> => {
	...
	return `/data/openalex${String(cacheFilePath)}`
}
